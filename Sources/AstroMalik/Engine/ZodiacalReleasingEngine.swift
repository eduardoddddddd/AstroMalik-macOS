...
final class ZodiacalReleasingEngine {
    ...
    private func birthDate(for chart: Chart, calendar: Calendar) throws -> Date {
        ...
        guard dateParts.count == 3, timeParts.count >= 2 else { throw ZodiacalReleasingError.invalidBirthData }
        ...
        guard let date = calendar.date(from: components) else { throw ZodiacalReleasingError.invalidBirthData }
        return date
    }
